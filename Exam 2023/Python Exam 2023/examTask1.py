...
#Created a function removeSensitiveWords using arguments as filenames to perform the operation
...
#Replace the sensitive words with asteriks "*" in the list.
 ...
 for sensitive_word in sensitive_wordlist:
     if sensitive_word in sensitive_wordlist: #Replaces lowercase words only. 
      ...
      #Replaces the word if the letter starts with an uppercase
     elif sensitive_word in sensitive_wordlist[0].upper: 
         rep_word = rep_word.replace(sensitive_word , asterisk)
...
removeSensitiveWords('censoredText.txt','sensitiveWordlist.txt','sensitiveRemoved.txt')  

#The changes is expected to be saved in output file :sensitiveRemoved.txt 
